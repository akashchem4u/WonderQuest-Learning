...
//
// Provides parent/guardian session management and access throttling that
// mirrors the pattern in child-access.ts. Keeps the parent auth durable
// across page navigations without requiring re-submission of credentials.

import { createHash, randomBytes } from "node:crypto";
import type { NextRequest } from "next/server";
import { db } from "@/lib/db";

export const PARENT_SESSION_COOKIE_NAME = "wonderquest-parent-session";

const ACCESS_TYPE_PARENT = "parent";

export class ParentAccessThrottleError extends Error {}

export class ParentAccessSessionError extends Error {}
...
function hashToken(token: string) {
  return createHash("sha256")
    .update(`wonderquest:${ACCESS_TYPE_PARENT}:${token}`)
    .digest("hex");
}
...
export async function assertParentAccessAllowed(
  identifier: string,
  ...
  const result = ipAddress
    ? await db.query(
        ...
        `,
        [ACCESS_TYPE_PARENT, identifier, ipAddress, recentSince],
      )
    : await db.query(
        `
          select
          ...
          where access_type = $1
            and identifier = $2
        `,
        [ACCESS_TYPE_PARENT, identifier, recentSince],
      );
  ...
  if (identifierFailures >= MAX_ATTEMPTS || ipFailures >= MAX_ATTEMPTS) {
    throw new ParentAccessThrottleError(
...
export async function clearParentAccessFailures(
  identifier: string,
  ipAddress: string | null,
) {
  if (ipAddress) {
    await db.query(
      ...
      `,
      [ACCESS_TYPE_PARENT, identifier, ipAddress],
    );
    return;
  }
  ...
  await db.query(
    `
      ...
      where access_type = $1
        ...
        and identifier = $2
    `,
    [ACCESS_TYPE_PARENT, identifier],
  );
}

export async function createParentAccessSession(input: {
  guardianId: string;
  ipAddress: string | null;
  userAgent: string | null;
}) {
...
export async function requireParentAccessSession(request: NextRequest) {
  const token = request.cookies.get(PARENT_SESSION_COOKIE_NAME)?.value ?? "";

  if (!token) {
    throw new ParentAccessSessionError(
      "A parent access session is required. Return to the parent hub and sign in again.",
    );
  }
  ...
  const result = await db.query(
    `
      ...
      where access_type = $1
        ...
        and expires_at > now()
      returning id, guardian_id, expires_at
    `,
    [ACCESS_TYPE_PARENT, hashToken(token)],
  );

  if (!result.rowCount) {
    throw new ParentAccessSessionError(
      "The parent session expired. Return to the parent hub and sign in again.",
    );
  }
