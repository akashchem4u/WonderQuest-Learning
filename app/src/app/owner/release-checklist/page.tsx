...
function statusLabel(s: ItemStatus): string {
  const map: Record<ItemStatus, string> = { complete: "Complete", in_progress: "In Progress", blocked: "Blocked", waived: "Waived", pending: "Pending" };
  return map[s];
}
...
function getCounts(items: CheckItem[]) {
  return {
    complete: items.filter((i) => i.status === "complete" || i.status === "waived").length,
...
function getOverall() {
  let total = 0; let complete = 0; let blocked = 0;
  for (const cat of CATEGORIES) {
    const c = getCounts(cat.items);
    total += c.total; complete += c.complete; blocked += c.blocked;
  }
...
export default function ReleaseChecklistPage() {
  ...
  const [expanded, setExpanded] = useState<Record<string, boolean>>({ engineering: true, content: true, accessibility: true, privacy: true, signoffs: true });
        ...
        <div style={{ maxWidth: "960px", margin: "0 auto", padding: "32px 20px 0" }}>
          ...
          <div style={{ display: "flex", gap: "4px", borderBottom: `1px solid ${BORDER}`, marginBottom: "28px", paddingTop: "20px" }}>
            {(["checklist", "history"] as const).map((t) => (
              <button key={t} onClick={() => setTab(t)} style={{ background: tab === t ? "rgba(80,232,144,.06)" : "none", border: "none", color: tab === t ? MINT : MUTED, fontFamily: "inherit", fontSize: "14px", fontWeight: 500, padding: "10px 18px", borderRadius: "6px 6px 0 0", cursor: "pointer", borderBottom: tab === t ? `2px solid ${MINT}` : "2px solid transparent" }}>
            ...
            <>
              ...
              {overall.blocked > 0 && (
                <div style={{ background: "rgba(232,80,80,.12)", border: "1px solid rgba(232,80,80,.35)", borderRadius: "6px", padding: "12px 16px", marginBottom: "20px", display: "flex", alignItems: "center", gap: "10px", fontSize: "13.5px", color: "#ff9090" }}>
                  ...
                  <span><strong>{overall.blocked} blocked item{overall.blocked > 1 ? "s" : ""}</strong> — Resolve before deploy.</span>
                </div>
              )}
                ...
                return (
                  <div key={cat.id} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: "10px", marginBottom: "14px", overflow: "hidden" }}>
                    <button
                      ...
                      style={{ display: "flex", alignItems: "center", gap: "12px", padding: "15px 20px", cursor: "pointer", background: "none", border: "none", width: "100%", textAlign: "left", color: TEXT, fontFamily: "inherit", fontSize: "14.5px", fontWeight: 600, minHeight: "52px" }}
                    >
                      <span style={{ fontSize: "17px" }}>{cat.icon}</span>
                      <span style={{ flex: 1 }}>{cat.name}</span>
                      <span style={{ fontSize: "12.5px", color: hasBl ? RED_ST : MUTED, fontWeight: 500 }}>
                        <span style={{ fontWeight: 700, color: hasBl ? RED_ST : MINT }}>{counts.complete}</span> / {counts.total} complete{hasBl ? ` · ${counts.blocked} blocked` : ""}
                      </span>
                      <span style={{ fontSize: "16px", color: MUTED, display: "inline-block", transform: isOpen ? "rotate(180deg)" : "rotate(0deg)", transition: "transform .2s", marginLeft: "4px" }}>▾</span>
                    </button>
          ...
          {tab === "history" && (
            <>
              ...
              {HISTORY.map((h) => {
                const isOpen = histOpen[h.version] ?? false;
                return (
                  <div key={h.version} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: "10px", marginBottom: "12px", overflow: "hidden" }}>
                    <button
                      onClick={() => setHistOpen((prev) => ({ ...prev, [h.version]: !prev[h.version] }))}
                      style={{ display: "flex", alignItems: "center", gap: "16px", padding: "16px 22px", cursor: "pointer", background: "none", border: "none", width: "100%", textAlign: "left", color: TEXT, fontFamily: "inherit", fontSize: "14.5px", fontWeight: 600, minHeight: "52px", flexWrap: "wrap" }}
                    >
