...
const INITIAL_WATCHLIST: WatchlistStudent[] = [
  {
    id: "jordan",
    initial: "J",
    color: "#475569",
    name: "Jordan",
    reason: "Confidence floor flag + personal check-in planned. Visual model suggested.",
    hasNote: true,
    chips: [
      { label: "Manually added", type: "manual" },
      { label: "P2 G2–3", type: "band" },
    ],
  },
  {
    id: "priya",
    ...
    color: "#ec4899",
    name: "Priya",
    reason: "Returned from 5-day absence. Monitor re-engagement over next week.",
    hasNote: false,
    chips: [
      { label: "Manually added", type: "manual" },
      { label: "P1 K–1", type: "violet" },
    ],
  },
  {
    id: "marcus",
    ...
    color: "#0ea5e9",
    name: "Marcus",
    reason: "Band ceiling — reviewing P3 advancement. Watching to confirm consistency.",
    hasNote: false,
    chips: [
      { label: "Manually added", type: "manual" },
      { label: "P2 → P3?", type: "advance" },
    ],
  },
];
...
export default function TeacherWatchlistPage() {
  const [students, setStudents] = useState<WatchlistStudent[]>(INITIAL_WATCHLIST);
      ...
      <div style={{ fontFamily: "system-ui,-apple-system,sans-serif", color: C.text, minHeight: "100vh", padding: "24px 28px" }}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 24 }}>
          <button style={tabStyle("watchlist")} onClick={() => setActiveTab("watchlist")}>
            Watchlist {students.length > 0 && `(${students.length})`}
          </button>
